"""
AEGIS Binary Integrity Trainer.

Version: 0.24.0

Empirical Stage-1 trainer for datasets such as Fakeddit that provide
binary authenticity/integrity supervision but do not provide valid
AEGIS threat subtype annotations.
"""
...
class BinaryIntegrityTrainer:
    """
    Train AEGIS alignment and Stage-1 integrity classification only.

    The existing hierarchical classifier is reused, but only its
    integrity logits participate in classification supervision.

    Threat logits may still be produced by the model architecture,
    but they are deliberately excluded from the empirical objective.
    """
    ...
    def __init__(
        ...
        if classification_loss_weight < 0:
            raise ValueError(
                "classification_loss_weight must be >= 0."
            )
    ...
    def forward_batch(
        ...
        return {
            ...
            # Explicitly absent from this task.
            "threat_loss": None,
    ...
    def train_step(
        ...
        """
        Execute one binary integrity optimization step.
        """
